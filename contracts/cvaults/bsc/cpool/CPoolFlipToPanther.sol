...
contract CPoolFlipToPanther is ICPool, VaultController, IStrategy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyCVaultBSC {
        require(msg.sender == cvaultBSC, 'CPoolFlipToPanther: caller is not the cvaultBSC');
        _;
    }
    ...
    function deposit(address to, uint amount, address _referrer) external override nonReentrant notPaused updateReward(to) onlyCVaultBSC {
        require(amount > 0, "CPoolFlipToPanther: amount must be greater than zero");
    ...
    function deposit(uint, address _referrer) public override {
        revert("N/A");
    }
    ...
    function getReward(address to) public override nonReentrant updateReward(to) onlyCVaultBSC {
        ...
        if (reward > 0) {
            ...
            emit ProfitPaid(to, pantherBalance, 0);
        }
    }
    ...
    function setRewardsToken(address newRewardsToken) public onlyOwner {
        require(address(_rewardsToken) == address(0), "CPoolFlipToPanther: rewards token already set");
    ...
    function _notifyRewardAmount(uint reward) private updateReward(address(0)) {
        ...
        require(rewardRate <= _balance.div(rewardsDuration), "CPoolFlipToPanther: reward rate must be in the right range");
    ...
    function setRewardsDuration(uint _rewardsDuration) external onlyOwner {
        require(periodFinish == 0 || block.timestamp > periodFinish, "CPoolFlipToPanther: reward duration can only be updated after the period ends");
    ...
    function recoverToken(address tokenAddress, uint tokenAmount) external override onlyOwner {
        require(tokenAddress != address(_stakingToken) && tokenAddress != _rewardsToken.stakingToken(), "CPoolFlipToPanther: cannot recover underlying token");
