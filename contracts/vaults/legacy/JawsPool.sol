...
contract JawsPool is IStrategyLegacy, RewardsDistributionRecipientUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    IBEP20 public constant stakingToken = IBEP20(0xdD97AB35e3C0820215bc85a395e13671d84CCBa2);   // jaws
    ...
    IBEP20 public rewardsToken; // jaws/bnb flip
    ...
    function initialize() external initializer {
        ...
        require(owner() != address(0), "JawsPool: owner must be set");
    ...
    function withdrawableBalanceOf(address account) override public view returns (uint) {
        if (block.timestamp > timestamp90DaysAfterPresaleEnds) {
            // unlock all presale jaws after 90 days of presale
            return _balances[account];
            ...
            uint jawsSupply = stakingToken.totalSupply().sub(stakingToken.balanceOf(deadAddress));
            if (soldInPresale >= jawsSupply) {
                return _balances[account].sub(_presaleBalance[account]);
            }
            uint jawsNewMint = jawsSupply.sub(soldInPresale);
            if (jawsNewMint >= soldInPresale) {
                return _balances[account];
            }

            uint lockedRatio = (soldInPresale.sub(jawsNewMint)).mul(1e18).div(soldInPresale);
    ...
    function apy() override public view returns(uint _usd, uint _jaws, uint _bnb) {
        ...
        uint jawsPrice = helper.tokenPriceInBNB(address(stakingToken));
        uint flipPrice = helper.tvlInBNB(address(rewardsToken), 1e18);

        _usd = 0;
        _jaws = 0;
        _bnb = rewardPerTokenPerSecond.mul(365 days).mul(flipPrice).div(jawsPrice);
    }
    ...
    function _flipToWBNB(uint amount) private returns(uint reward) {
        ...
        (uint rewardJaws,) = ROUTER.removeLiquidity(
            address(stakingToken), wbnb,
            amount, 0, 0, address(this), block.timestamp);
        address[] memory path = new address[](2);
        path[0] = address(stakingToken);
        path[1] = wbnb;
        ROUTER.swapExactTokensForTokens(rewardJaws, 0, path, address(this), block.timestamp);
